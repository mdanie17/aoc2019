...
func intCode(arr []int, input int) {
	i := 0
	for {
		ins := fmt.Sprintf("%5d", arr[i])

		parameterCode := [3]bool{
			ins[2] == ' ' || ins[2] == '0',
			ins[1] == ' ' || ins[1] == '0',
			ins[0] == ' ' || ins[0] == '0',
		}

		get := func(i, offset int) int {
			if parameterCode[offset-1] {
				return arr[arr[i+offset]]
			}
			return arr[i+offset]
		}

		op, _ := strconv.Atoi(strings.TrimSpace(ins[3:5]))

		if op == 1 {
			arr[arr[i+3]] = get(i, 1) + get(i, 2)
			i += 4
		}
		if op == 2 {
			arr[arr[i+3]] = get(i, 1) * get(i, 2)
